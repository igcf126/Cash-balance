...
def generar_archivo():
    ...
    analisis_pedidos_ya_df = agregar_contador(analisis_pedidos_ya_df, 'TIENDA_PED_YA', 'FECHA_PED_YA', 'NUMERO_ORDEN_PED_YA', 'CONTADOR_MONTO_PED_YA')
    analisis_pwr_df = agregar_contador(analisis_pwr_df, 'TIENDA_PWR', 'FECHA_PWR', 'NUMERO_ORDEN_PWR', 'CONTADOR_MONTO_PWR')
    ...
    # Fill NaN with 0 before converting to integer
    analisis_pedidos_ya_df['CONTADOR_MONTO_PED_YA'] = analisis_pedidos_ya_df['CONTADOR_MONTO_PED_YA'].fillna(0).astype(int)
    analisis_pwr_df['CONTADOR_MONTO_PWR'] = analisis_pwr_df['CONTADOR_MONTO_PWR'].fillna(0).astype(int)
    ...
    analisis_pedidos_ya_df = unir_columnas(analisis_pedidos_ya_df, 'TIENDA_PED_YA', 'FECHA_PED_YA', 'CONTADOR_MONTO_PED_YA', 'COD_MONTO_PED_YA')
    analisis_pwr_df = unir_columnas(analisis_pwr_df, 'TIENDA_PWR', 'FECHA_PWR', 'CONTADOR_MONTO_PWR', 'COD_MONTO_PWR')
    ...
    ## RECALCULAR COD INTERNOS

    cuadre_motado_df = pd.merge(analisis_pedidos_ya_df, analisis_pwr_df, left_on='COD_MONTO_PED_YA', right_on='COD_MONTO_PWR', how='outer')
    ...
    cuadre_motado_df['DIFERENCIA_DESCUENTO'] = (cuadre_motado_df['PRECIO_PWR'].fillna(0) - cuadre_motado_df['PRECIO_PED_YA'].fillna(0)).infer_objects(copy=False)
    ...
    # Create the columns TIENDA_UNICA and FECHA_UNICA
    cuadre_motado_df['TIENDA_UNICA'] = cuadre_motado_df['TIENDA_PWR'].combine_first(cuadre_motado_df['TIENDA_PED_YA'])
    cuadre_motado_df['FECHA_UNICA'] = cuadre_motado_df['FECHA_PWR'].combine_first(cuadre_motado_df['FECHA_PED_YA'])
    ...
    # Sum DIFERENCIA_DESCUENTO by TIENDA_UNICA and FECHA_UNICA
    diferencia_descuentos_totales2 = cuadre_motado_df.groupby(['TIENDA_UNICA', 'FECHA_UNICA'])['DIFERENCIA_DESCUENTO'].sum().reset_index()
    ...
    # Rename the columns to merge with merged_df
    diferencia_descuentos_totales2.rename(columns={
        'TIENDA_UNICA': 'TIENDA_SOFT',
        'FECHA_UNICA': 'FECHA_SOFT',
        'DIFERENCIA_DESCUENTO': 'DIFERENCIA_DESCUENTOS_TOTAL'
    }, inplace=True)
    ...
    print(analisis_cuadre_df.columns.tolist())
    
    cuadre_montado_df['CONTADOR_UNICO'] = cuadre_montado_df['CONTADOR_MONTO_PWR'].combine_first(cuadre_montado_df['CONTADOR_MONTO_PED_YA']).fillna(0).astype(int)
   ...
   # Calculate the difference in minutes between precio PWR and precio PED YA
    cuadre_montado_df['DIF'] = cuadre_montado_df.apply(
        lambda row: (row['PRECIO_PWR'] - row['PRECIO_PED_YA']),
        axis=1
    ).astype(int)
    ...
    cuadre_montado_df = cuadre_montado_df[
        ['TIENDA_UNICA', 'FECHA_UNICA', 'CONTADOR_UNICO', 'MINS', 'MATCH_PED_YA', 'VALIDACION_MATCH', 'VALIDACION_CANCELADAS',
         'TIENDA_PWR', 'FECHA_HORA_PWR', 'FECHA_PWR', 'HORA_PWR', 'TIPO_ORDEN_PWR', 'STATUS_ORDEN_PWR', 'NUMERO_ORDEN_PWR',
         'NOMBRE_CLT_PWR', 'TELEFONO_PWR', 'PRECIO_PWR', 'COMENTARIOS', 'CONTADOR_MONTO_PWR', 'COD_MONTO_PWR',
         'TIENDA_PED_YA', 'NOMBRE_TIENDA_PED_YA', 'FECHA_HORA_PED_YA', 'FECHA_PED_YA', 'HORA_PED_YA', 'NUMERO_ORDEN_PED_YA',
         'PRECIO_PED_YA', 'CONTADOR_MONTO_PED_YA', 'COD_MONTO_PED_YA', 'DIFERENCIA_DESCUENTO']]
    ...
    analisis_cuadre_df2 = analisis_cuadre_df.sort_values(by=['TIENDA_UNICA', 'FECHA_UNICA', 'DIFERENCIA_DESCUENTO']).reset_index(drop=True)

    cuadre_montado_df = cuadre_montado_df.sort_values(by=['TIENDA_UNICA', 'FECHA_UNICA', 'CONTADOR_UNICO']).reset_index(drop=True)
    ...
    if save_path:
        try:
            with pd.ExcelWriter(save_path, engine='openpyxl') as writer:
                ...
                analisis_cuadre_df.to_excel(writer, sheet_name='ANALISIS_PWR_PEDYA', index=False)
                cuadre_limpio_df.to_excel(writer, sheet_name='ANALISIS_TIENDA', index=False)
                cuadre_montado_df.to_excel(writer, sheet_name='ANALISIS_TIENDA por montos', index=False)
