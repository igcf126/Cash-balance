...
def generar_archivo():
    ...
    analisis_pwr_df.sort_values(by=['Store', 'Fecha', 'Final Order Price'], inplace=True)

    # PedidosYa analysis
    analisis_pedidos_ya_df = n_ord_ped_df[['Subtotal', 'Restaurant name', 'NUMERO', 'Order ID', 'Order received at', 'Fecha', 'Hora']].copy()
    analisis_pedidos_ya_df.sort_values(by=['NUMERO', 'Fecha', 'Subtotal'], inplace=True)
    ...
    def agregar_contador(df, columna_tienda, columna_fecha, columna_orden, nombre_columna_contador):
        df = df.sort_values(by=[columna_tienda, columna_fecha, columna_orden], ascending=[True, True, False]) # AGREGO EL CRITERIO DE QUE COLUMNA ORDEN, QUE TIENE EL MONTO, SEA EN ORDEN DESCENDENTE. IGCF
    ...
    # CAMBIADO EL CRITERIO DE CONTEO EN VEZ DE NUM DE PEDIDO, PRECIO
    analisis_pedidos_ya_df = agregar_contador(analisis_pedidos_ya_df, 'TIENDA_PED_YA', 'FECHA_PED_YA', 'PRECIO_PED_YA', 'CONTADOR_PED_YA') 
    analisis_pwr_df = agregar_contador(analisis_pwr_df, 'TIENDA_PWR', 'FECHA_PWR', 'PRECIO_PWR', 'CONTADOR_PWR')
    ...
    analisis_cuadre_df['DIF'] = analisis_cuadre_df.apply( # CAMBIO DE 'MINS' A DIF, IGCF
        lambda row: (row['PRECIO_PWR']) - (row['PRECIO_PED_YA']) if pd.notnull(row['FECHA_HORA_PWR']) and pd.notnull(row['FECHA_HORA_PED_YA']) else 0,
        axis=1
    ).astype(int)
    ...
    analisis_cuadre_df = analisis_cuadre_df[ # cambio la columna MINS por DIF. IGCF
        ['TIENDA_UNICA', 'FECHA_UNICA', 'CONTADOR_UNICO', 'DIF', 'MATCH_PED_YA', 'VALIDACION_MATCH', 'VALIDACION_CANCELADAS',
